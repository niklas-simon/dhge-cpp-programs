...
enum BtnEvents { EXIT, ONLY_TREE, ONLY_TEXT, BOTH, SAVE };
...
MyFrame::MyFrame()
        ...
        , splitter(new wxSplitterWindow(this, -1, wxDefaultPosition,
                                        wxSize(800, 600)))
        , directory(new wxGenericDirCtrl(splitter, -1))
        , text(new wxTextCtrl(splitter, -1, "", wxDefaultPosition,
                              wxDefaultSize,
                                      ...
                                      wxTE_PROCESS_TAB))
{
        ...
        Bind(wxEVT_COMMAND_BUTTON_CLICKED, &MyFrame::OnOnlyTree, this,
             ONLY_TREE);
        Bind(wxEVT_COMMAND_BUTTON_CLICKED, &MyFrame::OnOnlyText, this,
             ONLY_TEXT);
        ...
        if (text->IsModified() && !file.empty()) {
                if (!force) {
                        wxMessageDialog dlg = wxMessageDialog(
                                this,
                                "Do you want to save changes made to currently open file?",
                                "Warning", wxYES_NO | wxCENTRE);
