        ...
        for (int j = 0; j < length; j++) {
            if (!tiles[i][j]) {
                ...
                if (checkWon(tiles, turn, i, j) > 0) {
                    ...
                    int sum = sumMove(tiles, !(turn - 1) + 1, turn);
                    if (sum > max) {
                        max_move = {i, j};
                        max = sum;
                    }
...
class MyWindow : public wxWindow {
    public:
        MyWindow(wxWindow *parent): wxWindow(parent, -1, wxDefaultPosition, wxsize, wxBORDER_SIMPLE) {
            for (int i = 0; i < length; i++) {
                for (int j = 0; j < length; j++) {
        ...
        void draw(wxDC &dc) {
            ...
            for (int i = 0; i < length; i++) {
                if (i) {
                    ...
                    dc.DrawLine(i * tile_len, 0, i * tile_len, pixels - 1);
                    dc.DrawLine(0, i * tile_len, pixels - 1, i * tile_len);
                }
            }
            // draw marks
            for (int i = 0; i < length; i++) {
                for (int j = 0; j < length; j++) {
                    switch (tiles[i][j]) {
                        case 1:
                            dc.SetPen(*wxRED_PEN);
                            dc.DrawLine(i * tile_len + 20, j * tile_len + 20, (i + 1) * tile_len - 20, (j + 1) * tile_len - 20);
                            dc.DrawLine((i + 1) * tile_len - 20, j * tile_len + 20, i * tile_len + 20, (j + 1) * tile_len - 20);
                            break;
                        case 2:
                            dc.SetPen(*wxBLUE_PEN);
                            dc.DrawCircle(i * tile_len + tile_len / 2, j * tile_len + tile_len / 2, tile_len / 2 - 20);
                            break;
                    }
                }
            }
            ...
            if (won > 0) {
                ...
                switch (won) {
                    case 1:
                        // row
                        dc.DrawLine(30, lastY * tile_len + tile_len / 2, pixels - 30, lastY * tile_len + tile_len / 2);
                        break;
                    case 2:
                        // col
                        dc.DrawLine(lastX * tile_len + tile_len / 2, 30, lastX * tile_len + tile_len / 2, pixels - 30);
                        break;
                    case 3:
                        // diag 1
...
class MyFrame : public wxFrame {
    public:
        MyFrame() : wxFrame(nullptr, -1, "window"), puzzle(new MyWindow(this)) {
            ...
            Bind(wxEVT_COMMAND_BUTTON_CLICKED, &MyFrame::OnShuffle, this, CLEAR);
        }
