...
int main(int argc, char **argv)
{
        ...
        for (int i = 0; i < l_last; i++) {
                if (strlen(argv[i * 2 + 1]) != 5 ||
                    strlen(argv[i * 2 + 2]) != 5) {
                        fprintf(stderr,
                                "%s: guess and value must be of length 5\n",
                                argv[0]);
                        exit(EXIT_FAILURE);
                }
                last[i] = argv[i * 2 + 1];
                ...
                for (int j = 0; j < 5; j++) {
                        ...
                        switch (argv[i * 2 + 2][j]) {
                        case '0':
                                for (k = 0; k < 26 && absent[k] &&
                                            argv[i * 2 + 1][j] != absent[k];
                                     k++) {
                                }
                                if (k < 26 && !absent[k]) {
                                ...
                                break;
                        case '1':
                                for (k = 0; k < 5 && present[k] &&
                                            argv[i * 2 + 1][j] != present[k];
                                     k++) {
                                }
                                if (k < 26 && !present[k]) {
