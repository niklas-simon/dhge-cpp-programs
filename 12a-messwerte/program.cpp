...
class Values {
    private:
        ...
        double *values;
        Values(Values &o);
        Values(int length): length(length), values(new double[length]) {}
    public:
...
Values::Values(Values &o): length(o.length), values(new double[length]) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        cout << endl << "    length: " << v1.getLength() << " average: " << (double)v1 << endl;
        v += v1;
    }
    cout << "first: " << v[0] << " last: " << v[v.getLength() - 1] << " average: " << (double)v << " length: " << v.getLength() << endl;
}