...
template <typename T>
ostream &operator<<(ostream &stream, Values<T> &v);
...
class Values {
    private:
        ...
        T *values;
        Values(Values<T> &o);
        Values(int length): length(length), values(new T[length]) {}
    public:
        Values(): length(0), values(nullptr) {}
        ~Values() {
            delete[] values;
        }
...
template <typename T>
Values<T>::Values(Values<T> &o): length(o.length), values(new T[length]) {
...
template <typename T>
Values<T> &Values<T>::operator+=(T value) {
...
template <typename T>
Values<T> &Values<T>::operator+=(Values<T> &v) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        cout << endl << "    length: " << v1.getLength() << " average: " << (Fraction)v1 << endl;
        v += v1;
    }
    cout << "first: " << v[0] << " last: " << v[v.getLength() - 1] << " average: " << (Fraction)v << " length: " << v.getLength() << endl;
}