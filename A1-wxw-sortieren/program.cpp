...
const wxString initList[] = { "Aufsteigend",        "Absteigend",
                              "Durcheinander",      "Fast sortiert",
                              "Sortiert mit Neuen", "Lauter Gleiche" };
const int initSize = sizeof(initList) / sizeof(initList[0]);
// Auswahlliste für das Sortierverfahren
const wxString sortList[] = { "Selection Sort",
                              "Insertion Sort",
                              "Bubblesort",
                              "Bi-Bubblesort",
                              "Shellsort",
                              "Heapsort",
                              "Quicksort",
                              "Quicksort (verzoegert)",
                              "Quicksort (einfaches Part)",
                              "Quicksort (best-of-3 Part)" };
...
{
        wxPaintDC dc(
                this); // im Repaint muss es ein wxPaintDC statt wxClientDC sein
...
void Array::DrawPoint(int i)
{
        ...
        if (deleted[i]) {
                ...
                if ((i < arrSize - 1) && !deleted[i + 1] &&
                    (abs(data[i + 1] - data[i]) <= 2)) {
                        DrawPoint(i + 1);
                }
                if ((i < arrSize - 2) && !deleted[i + 2] &&
                    (abs(data[i + 2] - data[i]) <= 2)) {
                        DrawPoint(i + 2);
                }
        }
