...
double solve(Item *items, int size, int index, double max_weight, double curr_weight, double curr_value, double optimal) {
    calls++;
    if (index >= size) {
    ...
    optimal = solve(items, size, index + 1, max_weight, curr_weight, curr_value, optimal);
    if (curr_weight + items[index].weight < max_weight) {
        items[index].curr_packed = true;
        optimal = solve(items, size, index + 1, max_weight, curr_weight + items[index].weight, curr_value + items[index].value, optimal);
    }
    return optimal;
}
...
int main(int argc, char **argv) {
    ...
    for (count = 0; count < MAX_ITEMS; count++) {
        int res = fscanf(input, "%s %lf %lf\n", items[count].name, &(items[count].value), &(items[count].weight));
        if (res == -1) {
            break;
        }
        if (res < 3) {
            fprintf(stderr, "%s: error while reading from file %s: invalid structure: %d\n", argv[0], argv[1], res);
            exit(EXIT_FAILURE);
        }
