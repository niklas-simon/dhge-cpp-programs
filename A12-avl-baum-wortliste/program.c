...
void print_tree(word_entry *p, int layer, FILE *file)
{
        if (!p)
                return;
        for (int i = 0; i < layer; i++)
                fprintf(file, "  ");
        fprintf(file, "%s (%i)\n", p->word, p->balance);
        print_tree(p->left, layer + 1, file);
        print_tree(p->right, layer + 1, file);
}

void rotate_right(word_entry **p)
{
        //print_tree(root, 0, stdout);
        //printf("rotating right (%s and %s)\n", (*p)->word, (*p)->right->word);
        word_entry *t = (*p)->right->left;
        (*p)->right->left = *p;
        *p = (*p)->right;
        (*p)->left->right = t;
}

void rotate_left(word_entry **p)
{
        //print_tree(root, 0, stdout);
        //printf("rotating left (%s and %s)\n", (*p)->word, (*p)->left->word);
        word_entry *t = (*p)->left->right;
        (*p)->left->right = *p;
        *p = (*p)->left;
        (*p)->right->left = t;
}

int helper(word_entry **p, pos_entry *pos, const char *word)
{
        if (!*p) {
                *p = new_word(word, pos);
                return 1;
        ...
        int res;
        if (cmp < 0) {
                res = helper(&((*p)->right), pos, word);
                (*p)->balance += res;
        } else {
                res = helper(&((*p)->left), pos, word);
                (*p)->balance -= res;
        }
        if ((*p)->balance > 1) {
                // rechts
                int bal = (*p)->right->balance;
                if (bal >= 0) {
                        // einfach
                        rotate_right(p);
                        (*p)->balance = 0;
...
{
        word_entry *p = root;
        for (; p && strcmp(p->word, word) != 0;
             p = strcmp(p->word, word) < 0 ? p->right : p->left) {
        }
        if (p) {
                list_pos(p);
        } else {
...
void get_depth(word_entry *knot, int layer, int *max_depth, int *sum_depth,
               int *leaf_count)
{
        if (!knot->left && !knot->right) {
                ...
                *leaf_count += 1;
                ...
                if (knot->left) {
                        get_depth(knot->left, layer + 1, max_depth, sum_depth,
                                  leaf_count);
                }
                if (knot->right) {
                        get_depth(knot->right, layer + 1, max_depth, sum_depth,
                                  leaf_count);
                }
        }
}
