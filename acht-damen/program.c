        ...
        for (int col = 0; col < 8; col++) {
                if (!((0x1LL << (row * 8 + col)) & guarded)) {
                        newqueens =
                                ...
                                (0x1LL
                                 << (row * 8 +
                                     col)); // insert queen where she belongs
                        newguarded =
                                guarded |
                                (0xffLL
                                 << (row
                                     << 3)) // row of ones shifted into row where queen is (i / 8)
                                |
                                (0x0101010101010101LL
                                 << col); // col of ones shifted into col where queen is (i % 8)
                        if (col > row) {
                                // shift diag-pattern difference rows up
                                newguarded |= 0x8040201008040201LL >>
                                              ((col - row) << 3);
                        } else {
                        ...
                        if ((7 - col) > row) {
                                ...
                                newguarded |= 0x102040810204080LL >>
                                              (((7 - col) - row) << 3);
                        } else {
