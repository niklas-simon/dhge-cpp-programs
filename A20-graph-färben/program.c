...
Point coords(int city) {
    ...
    double len = (SDL_X_SIZE < SDL_Y_SIZE ? SDL_X_SIZE : SDL_Y_SIZE) * 1 / 3;
        ...
        for (int j = 0; j < COUNT; j++) {
            if (matrix[i][j]) {
                ...
                for (int k = 0; k < STEPS - 1; k++) {
                    ...
                    sdlDrawLine(c1.pos.x + dist_x * k / STEPS, c1.pos.y + dist_y * k / STEPS, c1.pos.x + dist_x * (k + 1) / STEPS, c1.pos.y + dist_y * (k + 1) / STEPS, c.r * 255, c.g * 255, c.b * 255);
                }
            }
        }
        sdlDrawCirc(c1.pos.x, c1.pos.y, 5, 5, color.r * 255, color.g * 255, color.b * 255);
    }
    sdlUpdate();
}
...
void step(City *cities, double force) {
    for (int i = 0; i < COUNT; i++) {
        for (int j = 0; j < COUNT; j++) {
            ...
            double l = length(d);
            if (l < 100) {
                cities[i].direction.x -= .5 * (cities[j].pos.x - cities[i].pos.x);
                cities[i].direction.y -= .5 * (cities[j].pos.y - cities[i].pos.y);
            } else if (matrix[i][j]) {
                cities[i].direction.x += force * (cities[j].pos.x - cities[i].pos.x);
                cities[i].direction.y += force * (cities[j].pos.y - cities[i].pos.y);
            } else if (l < 200) {
                cities[i].direction.x -= 2 * force * (cities[j].pos.x - cities[i].pos.x);
                cities[i].direction.y -= 2 * force * (cities[j].pos.y - cities[i].pos.y); 
            }
        }
    }
    for (int i = 0; i < COUNT; i++) {
...
void fit(City *cities) {
    ...
    double max = length(distance(m, SDL_X_SIZE < SDL_Y_SIZE ? (Point){SDL_X_SIZE, m.y} : (Point){m.x, SDL_Y_SIZE}));
    for (int i = 0; i < COUNT; i++) {
    ...
    for (;;) {
        for (int i = 0; i < COUNT; i++) {
            Point p = (Point){cities[i].pos.x + cities[i].direction.x, cities[i].pos.y + cities[i].direction.y};
            if (p.x < 10 || p.y < 10 || p.x >= SDL_X_SIZE - 10 || p.y >= SDL_Y_SIZE - 10) {
                return;
            }
            cities[i].pos = p;
        }
...
void visualize(int color_count) {
    ...
    City cities[COUNT];
    for (int i = 0; i < COUNT; i++) {
        cities[i] = (City){coords(i), (Point){0, 0}, hue(colors[i], color_count)};
    }
