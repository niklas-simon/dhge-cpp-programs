...
void eingabe(int argc, const char *argv[]) {
    ...
    for (i = wort_len - 1; i >= 0; --i) {
        op1[i] = (i < wort_len - l1) ? 10 : buchstabe(argv[1][i - (wort_len - l1)]);
        op2[i] = (i < wort_len - l2) ? 10 : buchstabe(argv[2][i - (wort_len - l2)]);
        sum[i] = (i < wort_len - l3) ? 10 : buchstabe(argv[3][i - (wort_len - l3)]);
    }
...
bool sinnvoll(void) {
    ...
    for (int i = wort_len - 1; i >= 0; i--) {
        ...
        if (ziffer[op1[i]] == -1 || ziffer[op2[i]] == -1 || ziffer[sum[i]] == -1) {
...
void ausgabe(void) {
    ...
    for (i = 0; i < buch_anz; ++i) {
        printf("%c=%d%s", buchst[i], ziffer[i], (i == buch_anz - 1) ? "\n" : ", ");
    }
    print_wort(op1);
    print_wort(op2);
    print_wort(sum);
