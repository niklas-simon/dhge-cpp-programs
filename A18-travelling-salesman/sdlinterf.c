...
void sdlInit(void)
{
        ...
        myWindow = SDL_CreateWindow("SDL Graphics", SDL_WINDOWPOS_UNDEFINED,
                                    SDL_WINDOWPOS_UNDEFINED, SDL_X_SIZE,
                                    SDL_Y_SIZE, 0);
        if (myWindow == NULL) {
        ...
        myTexture = SDL_CreateTexture(myRenderer, SDL_PIXELFORMAT_ARGB8888,
                                      SDL_TEXTUREACCESS_STREAMING, SDL_X_SIZE,
                                      SDL_Y_SIZE);
        if (myTexture == NULL) {
                fprintf(stderr, "Could not create SDL Renderer: %s\n",
                        SDL_GetError());
...
void sdlSetBlack(void)
{
        ...
        for (Uint32 *p = myPixArray; p < myPixArray + SDL_X_SIZE * SDL_Y_SIZE;
             ++p) {
                *p = pixel;
        }
...
{
        if ((x < 0) || (x >= SDL_X_SIZE) || (y < 0) || (y >= SDL_Y_SIZE)) {
...
void sdlDrawRect(int centerX, int centerY, int extX, int extY, int r, int g,
                 int b)
{
        ...
        if ((extX < 0) || (extY < 0) || (firstX < 0) || (firstY < 0) ||
        ...
        for (Uint32 *p1 = myPixArray + firstY * SDL_X_SIZE + firstX; p1 <= e1;
             p1 += SDL_X_SIZE) {
...
void sdlDrawRectFromTo(int X1, int Y1, int X2, int Y2, int r, int g, int b)
{
        ...
        if ((firstX < 0) || (firstY < 0) || (lastX >= SDL_X_SIZE) ||
            (lastY >= SDL_Y_SIZE)) {
                fprintf(stderr, "Rect (%d/%d ... %d/%d) offscreen.\n", X1, Y1,
                        X2, Y2);
                exit(EXIT_FAILURE);
...
{
        if ((x1 < 0) || (x1 >= SDL_X_SIZE) || (y1 < 0) || (y1 >= SDL_Y_SIZE) ||
            (x2 < 0) || (x2 >= SDL_X_SIZE) || (y2 < 0) || (y2 >= SDL_Y_SIZE)) {
...
void sdlDrawCirc(int centerX, int centerY, int radX, int radY, int r, int g,
                 int b)
{
...
void sdlDrawCircPart(int centerX, int centerY, int radX, int radY, int r, int g,
                     int b, int part)
{
        ...
        if ((radX < 0) || (radY < 0) || (leftX < 0) || (upperY < 0) ||
        ...
        if ((radX == 0) && (radY == 0)) {
                ...
                while (stopX <= stopY) {
                        ...
                        chgX += sqrY;
                        if (2 * err + chgY > 0) {
                                drawCircLine(centerX, centerY, x - 1, oldX, y,
                                             pixel, part);
                                oldX = x;
                                --y;
                                stopY -= sqrX;
                ...
                for (; y > lastY; --y) {
                        drawCircLine(centerX, centerY, x - 1, oldX, y, pixel,
                                     part);
                }
                if ((lastX > x) && !(part & SDL_CIRC_FILLED)) {
                        drawCircLine(centerX, centerY, lastX - 1, x, lastY,
                                     pixel, part);
                }
        }
        ...
        if (maxX > MINVAL) {
                ...
                if ((updRect.w == SDL_X_SIZE) && (updRect.h == SDL_Y_SIZE)) {
                        ...
                        if (SDL_UpdateTexture(myTexture, NULL, myPixArray,
                                              sizeof(Uint32) * SDL_X_SIZE) !=
                            0) {
                                fprintf(stderr,
                                        "Could not update SDL texture: %s.\n",
                                        SDL_GetError());
                                exit(EXIT_FAILURE);
                        }
                        if (SDL_RenderCopy(myRenderer, myTexture, NULL, NULL) !=
                            0) {
                                fprintf(stderr,
                                        "Could not display SDL texture: %s.\n",
                                        SDL_GetError());
                                exit(EXIT_FAILURE);
                        }
                } else {
                        ...
                        if (SDL_RenderCopy(myRenderer, myTexture, &updRect,
                                           &updRect) != 0) {
                                fprintf(stderr,
                                        "Could not display SDL texture: %s.\n",
                                        SDL_GetError());
